...
func TestGetBoundingBox(t *testing.T) {
	...
	for i := 0; i < len(fc.Features); i++ {
		f := fc.Features[i]
		min, max, ok := f.GetBoundingBox()
		switch f.GetBoundingBox {
		case GeometryPoint:
			if !ok || min != [2]float64{102.0, 0.5} || max != [2]float64{102.0, 0.5} {
				t.Errorf("should have Point Boundingbox is [{102.0, 0.5},{102.0, 0.5}], got %v,%v", min, max)
			}
		case GeometryLineString:
			if !ok || min != [2]float64{102.0, 0.0} || max != [2]float64{105.0, 1.0} {
				t.Errorf("should have LineString Boundingbox is [{102.0, 0.0},{105.0, 1.0}], got %v,%v", min, max)
			}
		case GeometryPolygon:
			if !ok || min != [2]float64{100.1, 0.0} || max != [2]float64{101.5, 1.0} {
				t.Errorf("should have Point Boundingbox is [{100.1, 0.0},{101.5, 1.0}], got %v,%v", min, max)
			}
		}
	}

	min, max, ok := fc.GetBoundingBox()
	if !ok || min != [2]float64{100.1, 0.0} || max != [2]float64{105.0, 1.0} {
